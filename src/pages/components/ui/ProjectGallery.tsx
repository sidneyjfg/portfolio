  ...
  return (
    <>
      <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3">
        {images.map((src, i) => (
          <motion.button key={src} onClick={() => setOpenIdx(i)} whileHover={{ scale: 1.02 }}
      ...
      <AnimatePresence>
        {openIdx !== null && (
          <motion.div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
            ...
            {images.length > 1 && (
              <>
                <button onClick={(e) => { e.stopPropagation(); setOpenIdx((i) => (i! - 1 + images.length) % images.length); }}
                        className="absolute left-6 top-1/2 -translate-y-1/2 text-white/80 text-3xl" aria-label="Anterior">‹</button>
                <button onClick={(e) => { e.stopPropagation(); setOpenIdx((i) => (i! + 1) % images.length); }}
                        className="absolute right-6 top-1/2 -translate-y-1/2 text-white/80 text-3xl" aria-label="Próxima">›</button>
              </>
            )}
          </motion.div>
        )}
